...
class TestModel(nn.Module):
    ...
    def forward(self, input):
        val = self.enc(input)
        val = self.CGRU1(val, future=3, return_only_final=False)
        val = self.CGRU2(val, future=3, return_only_final=True)
        val = self.CGRU3(val, future=3, return_only_final=True)
        val = self.dec(val)
        return val
...
generator_train = load_car_data(bbs=100, skip=5)
generator_test  = load_car_data(bbs=10, skip=5)
